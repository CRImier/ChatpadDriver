...
class Chatpad():
    ...
    def __init__(self, port = "/dev/ttyAMA0", name="xbox_chatpad_input", callback = None):
        if callback == None:
            callback = lambda *args: None #Empty callback
        self.callback = callback
        self.name = name
        self.port = port
        self.serial = Serial(port, 19200)
        if name: self.uinput = UInput(name=name, devnode='/dev/uinput')
    ...
    def thread_listen(self):
        ...
        while True:
            while self.serial.inWaiting() > 8:
                ...
                held = [key for key in previous_keys if key in pressed_keys]
                for key in held:
                    print "Key held: "+self.key_dict[key]
                self.uinput.syn()
                self.callback(pressed, released, held)
